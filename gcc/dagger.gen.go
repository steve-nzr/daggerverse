...
func (r GCC) MarshalJSON() ([]byte, error) {
	var concrete struct {
		DebianVersion   string
		GccVersion      string
		SourceDirectory *Directory
	}
	concrete.DebianVersion = r.DebianVersion
	concrete.GccVersion = r.GccVersion
	concrete.SourceDirectory = r.SourceDirectory
	return json.Marshal(&concrete)
}

func (r *GCC) UnmarshalJSON(bs []byte) error {
	var concrete struct {
		DebianVersion   string
		GccVersion      string
		SourceDirectory *Directory
	...
	r.DebianVersion = concrete.DebianVersion
	r.GccVersion = concrete.GccVersion
	r.SourceDirectory = concrete.SourceDirectory
	return nil
}
...
func invoke(ctx context.Context, parentJSON []byte, parentName string, fnName string, inputArgs map[string][]byte) (_ any, err error) {
	...
	case "GCC":
		...
		case "Container":
			...
			return (*GCC).Container(&parent), nil
		case "":
			var parent GCC
			err = json.Unmarshal(parentJSON, &parent)
			if err != nil {
			...
			if inputArgs["debianVersion"] != nil {
				err = json.Unmarshal([]byte(inputArgs["debianVersion"]), &debianVersion)
				if err != nil {
					panic(fmt.Errorf("%s: %w", "failed to unmarshal input arg debianVersion", err))
				}
			}
			var gccVersion string
			if inputArgs["gccVersion"] != nil {
				err = json.Unmarshal([]byte(inputArgs["gccVersion"]), &gccVersion)
				if err != nil {
					panic(fmt.Errorf("%s: %w", "failed to unmarshal input arg gccVersion", err))
				}
			}
			var sourceDirectory *Directory
			if inputArgs["sourceDirectory"] != nil {
				err = json.Unmarshal([]byte(inputArgs["sourceDirectory"]), &sourceDirectory)
				if err != nil {
					panic(fmt.Errorf("%s: %w", "failed to unmarshal input arg sourceDirectory", err))
				}
			}
			return New(debianVersion, gccVersion, sourceDirectory), nil
		default:
			return nil, fmt.Errorf("unknown function %s", fnName)
		}
	case "":
		return dag.Module().
			WithDescription("A generated module for GCC functions\n\nThis module has been generated via dagger init and serves as a reference to\nbasic module structure as you get started with Dagger.\n\nTwo functions have been pre-created. You can modify, delete, or add to them,\nas needed. They demonstrate usage of arguments and return types using simple\necho and grep commands. The functions can be called from the dagger CLI or\nfrom one of the SDKs.\n\nThe first line in this comment block is a short description line and the\nrest is a long description with more detail on the module's purpose or usage,\nif appropriate. All modules should have a short description.\n").
			WithObject(
				dag.TypeDef().WithObject("GCC").
					WithFunction(
						dag.Function("Run",
							dag.TypeDef().WithKind(StringKind)).
							WithDescription("Run your program in a container\n\nExample:\n$ dagger call --source-directory=$HOME/dev/cpp/src run")).
					WithFunction(
						dag.Function("Container",
							dag.TypeDef().WithObject("Container")).
							WithDescription("Creates a container from your program\n\nThis will compile and put your application inside a distroless container\nthat you can publish or run\n\nExample:\n$ dagger call --source-directory=$HOME/dev/cpp/src container publish --address=\"$addr\"")).
					WithField("DebianVersion", dag.TypeDef().WithKind(StringKind)).
					WithField("GccVersion", dag.TypeDef().WithKind(StringKind)).
					WithField("SourceDirectory", dag.TypeDef().WithObject("Directory")).
					WithConstructor(
						dag.Function("New",
							dag.TypeDef().WithObject("GCC")).
							WithArg("debianVersion", dag.TypeDef().WithKind(StringKind).WithOptional(true), FunctionWithArgOpts{DefaultValue: JSON("\"bookworm\"")}).
							WithArg("gccVersion", dag.TypeDef().WithKind(StringKind).WithOptional(true), FunctionWithArgOpts{DefaultValue: JSON("\"12\"")}).
							WithArg("sourceDirectory", dag.TypeDef().WithObject("Directory"), FunctionWithArgOpts{Description: "Your C++ project directory. It must contain a main.cpp"}))), nil
	default:
