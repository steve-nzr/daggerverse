...
func (r WWHRD) MarshalJSON() ([]byte, error) {
	var concrete struct {
		WWHRDVersion string
		GoVersion    string
	}
	concrete.WWHRDVersion = r.WWHRDVersion
	concrete.GoVersion = r.GoVersion
	return json.Marshal(&concrete)
}

func (r *WWHRD) UnmarshalJSON(bs []byte) error {
	var concrete struct {
		WWHRDVersion string
		GoVersion    string
	}
	...
	r.WWHRDVersion = concrete.WWHRDVersion
	r.GoVersion = concrete.GoVersion
	return nil
}
...
func invoke(ctx context.Context, parentJSON []byte, parentName string, fnName string, inputArgs map[string][]byte) (_ any, err error) {
	...
	case "WWHRD":
		...
		case "Check":
			...
			return (*WWHRD).Check(&parent, ctx, sourceDir)
		case "":
			var parent WWHRD
			err = json.Unmarshal(parentJSON, &parent)
			if err != nil {
			...
			var wwhrdVersion string
			if inputArgs["wwhrdVersion"] != nil {
				err = json.Unmarshal([]byte(inputArgs["wwhrdVersion"]), &wwhrdVersion)
				if err != nil {
					panic(fmt.Errorf("%s: %w", "failed to unmarshal input arg wwhrdVersion", err))
				}
			}
			...
			return New(wwhrdVersion, goVersion), nil
		default:
	...
	case "":
		return dag.Module().
			WithObject(
				dag.TypeDef().WithObject("WWHRD").
					WithFunction(
						dag.Function("Check",
							dag.TypeDef().WithKind(StringKind)).
							WithDescription("Runs the check command of wwhrd\n\nExample:\n$ dagger call --go-version=1.22 --wwhrd-version=latest check --source-dir=$HOME/my-go-project").
							WithArg("sourceDir", dag.TypeDef().WithObject("Directory"))).
					WithField("WWHRDVersion", dag.TypeDef().WithKind(StringKind)).
					WithField("GoVersion", dag.TypeDef().WithKind(StringKind)).
					WithConstructor(
						dag.Function("New",
							dag.TypeDef().WithObject("WWHRD")).
							WithArg("wwhrdVersion", dag.TypeDef().WithKind(StringKind).WithOptional(true), FunctionWithArgOpts{DefaultValue: JSON("\"latest\"")}).
							WithArg("goVersion", dag.TypeDef().WithKind(StringKind).WithOptional(true), FunctionWithArgOpts{DefaultValue: JSON("\"1.22\"")}))), nil
	default:
